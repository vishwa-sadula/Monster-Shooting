...
let color=["blue", "green", "purple", "orange", "red"];
...
class Genetics {
	...
	createPopulation(){
		...
		for(let i = 0; i < this.populationSize; i++){
			...
			this.population.push( new Invader( w/4/2 ,Math.random()*-20, shape, color[ Math.floor(Math.random()*5)]) );
		}
	}
	...
	crossOver(_a,_b){
		...
		let child = new Invader(x,Math.random()*-20,a.shape.slice(), a.color);
	...
	elitism(){
		...
		let invader = new Invader(w/4/2 ,Math.random()*-20, this.bestOfGeneration.shape.slice(), this.bestOfGeneration.color);
